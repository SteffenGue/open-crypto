...
class TestPersistResponse(unittest.TestCase):
    ...
    def test_persist_valid_ticker(self):
        ...
        response2 = [(ExCuPair[0], ExCuPair[1], ExCuPair[2]) + res for (ExCuPair, res) in zip(self.exchange_currency_pairs, response)]
    ...
    def test_persist_response_with_unknown_column(self):
        ...
        exchange = list(exchanges_with_pairs.keys())[0]
        request_name = Ticker.__tablename__
        mappings = ['start_time', 'time', 'best_ask', 'best_bid', 'last_price', 'daily_volume', 'exchange_pair_id', 'some_column']
